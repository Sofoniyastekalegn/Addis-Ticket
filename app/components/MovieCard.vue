...
<template>
    <div class="group relative bg-dark-900 rounded-lg overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
      ...
      <div class="p-4">
        ...
        <!-- Showtimes -->
        <div v-if="movie.schedules && movie.schedules.length > 0" class="space-y-2">
          <p class="text-xs text-gray-500 uppercase tracking-wide">Showtimes</p>
          <div class="flex flex-wrap gap-1">
            <span 
              v-for="schedule in movie.schedules.slice(0, 3)" 
              :key="schedule.id"
              class="text-xs bg-primary-500/20 text-primary-400 px-2 py-1 rounded"
            >
              {{ formatTime(schedule.start_time) }}
            </span>
            <span v-if="movie.schedules.length > 3" class="text-xs text-gray-500">
